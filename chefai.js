...
export default async function handler(req, res) {
  ...
  try {
    const { mode = "recipes", sign, state, mealType, lang = "fr", message } = req.body || {};

    // --- 1) MODE RECETTES (gold_ai.html principal) ---
    if (mode === "recipes") {
      if (!sign || !mealType) {
        return res.status(400).json({ error: "Missing sign or mealType" });
      }
